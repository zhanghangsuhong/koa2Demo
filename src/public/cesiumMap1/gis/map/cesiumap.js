        ...
        */      
        ...
        $("#cesium3DTiles").click(function(){
            if($(this).attr("class").indexOf("selected") != -1){
                ...
                $("#showLists3DtilesDIV").show();
            }else{
                cesium.remove3DTiles();
                $("#showLists3DtilesDIV").hide();
                $("#cesiumClearData").click();
            }
        });

        /**
         *加载倾斜模型单体化3DTiles信息列表
        */      
       function load3DtilesList(){
            var data = MapConfig.Obj3Dtilesjson;
            if(data && data.length>0){
                var innerStr = [];
                for(var i=0;i<data.length;i++){
                    innerStr.push('<div class="left_list_li_box" id = "' + data[i].id + '">');
                ...
                $("#showLists3Dtiles").html(innerStr.join(''));
                $(".left_list_li_box").click(function(e){
                    //console.log("left_list_li_box",e.currentTarget.id);
                    var id = e.currentTarget.id;
                    toLocation3Dtiles(id);
                });
                //滚动条样式
                $("#showLists3Dtiles").mCustomScrollbar({
                    theme: "minimal-dark",
                });
            }           
        }
        ...
        /**
         * 根据id匹配对应的geoserver图层服务数据
         * @param  id 
         */      
        function toLocation3Dtiles(id){
            queryWFSData({
                typeName: MapConfig.geoserverTypeName,
                propertyName: MapConfig.geoserverPropertyId,//id
        ...
        handler3D.setInputAction(function(movement) {
            ...
            else if((pick && pick.primitive) || pick instanceof Cesium.Cesium3DTileFeature){ //倾斜模型点击
                var cartesian = cesium.cesiumViewer.scene.pickPosition(movement.position);
                var lnglat = cesium.cartesianToLnglat(cartesian);//坐标转换
                //console.log('点击倾斜模型',lnglat);
                cesium.queryWFSBylnglats([lnglat]);
            }
            else{
                $('#trackPopUp').hide();
            }
        ...
        //地图查询
        $("#cesiumQueryMap").click(function(){
            $("#showListsQueryMapDIV").show();
            //跳转位置
            cesium.flyToPosition(Cesium.Cartesian3.fromDegrees(113.370932, 23.097739,999));
        });
        $("#CloseImg").click(function(){
            $("#cesiumClearData").click();
            $("#showListsQueryMap").empty();
            $("#showListsQueryMapDIV").hide();
        });
        //属性查询
        $("#research_btn").click(function(){
            ...
            queryWFSData({
                typeName: MapConfig.geoserverTypeName,
                propertyName: MapConfig.geoserverPropertyName,//name
                url: MapConfig.geoserverUrl + MapConfig.geoserverWorkspace + "/ows",
                queryData: {
                ...
                success: function (data) {
                    ...
                    cesium.removeAllDss();
                    cesium.removeAllPopup();
                    cesium.loadGeojson(data,true);

                    var innerStr = [];
                    for(var i=0;i<data.features.length;i++){
                        innerStr.push('<div class="left_list_li_box" id = "' + data.features[i].properties.Id + '">');
                        innerStr.push('<div class="left_list_li_box_top">');
                        innerStr.push('<div class="left2_box2">');
                        innerStr.push('<img class="list_poi_marker" src="./gis/cesium/images/poiLocation.png"></img>');
                        innerStr.push('<div class="left_list_li1">');
                        innerStr.push('<p>');
                        innerStr.push('<a>id:' + data.features[i].properties.Id + '</a><br/>');
                        innerStr.push('<a>名称:' + data.features[i].properties.name + '</a><br/>');
                        innerStr.push('<a>地址:' + data.features[i].properties.address + '</a><br/>');
                        innerStr.push('</p>');
                        innerStr.push('</div>');
                        innerStr.push('</div>')
                        innerStr.push('</div>');
                    ...
                    $("#showListsQueryMap").html(innerStr.join(''));
                    $(".left_list_li_box").click(function(e){
                        var id = e.currentTarget.id;
                        toLocation3Dtiles(id);
                    });
                    //滚动条样式
                    $("#showListsQueryMap").mCustomScrollbar({
                        theme: "minimal-dark",
                    });
        ...
        $("#drawRQuery").click(function(){
            ...
            drawTool.startDraw({
                ...
                success: function (evt) {
                    ...
                    //世界坐标转地理坐标（弧度）
                    var leftupcartographic = cesium.cesiumViewer.scene.globe.ellipsoid.cartesianToCartographic(leftup);
                    var rightdowncartographic = cesium.cesiumViewer.scene.globe.ellipsoid.cartesianToCartographic(rightdown);
                    //地理坐标（弧度）转经纬度坐标
                    var leftuppoint=[ Cesium.Math.toDegrees(leftupcartographic.longitude), Cesium.Math.toDegrees(leftupcartographic.latitude)];
                    var rightdownpoint=[ Cesium.Math.toDegrees(rightdowncartographic.longitude), Cesium.Math.toDegrees(rightdowncartographic.latitude)];
                    var extent = [leftuppoint[0].toFixed(6),leftuppoint[1].toFixed(6),rightdownpoint[0].toFixed(6),rightdownpoint[1].toFixed(6)];
                    var lnglats = [];
                    if(extent && extent.length>0){
                        lnglats.push([extent[0],extent[1]]);
                        lnglats.push([extent[2],extent[1]]);
                        lnglats.push([extent[2],extent[3]]);
                        lnglats.push([extent[0],extent[3]]);
                    }
                    console.log(lnglats);
                    loadSpatialQueryResults(lnglats);
                }
            });
        });
        //绘制多边形查询
        $("#drawQuery").click(function(){
            ...
            drawTool.startDraw({
                ...
                success: function (evt) {
                    var lnglats = [];
                    var positions = evt.positions;
                    for (var i = 0; i < positions.length; i++) {
                        var jwd = cesium.cartesianToLnglat(positions[i]);
                        lnglats.push(jwd);
                    }
                    console.log(lnglats);
                    loadSpatialQueryResults(lnglats);
        ...
        //空间查询结果展示
        function loadSpatialQueryResults(lnglats){
